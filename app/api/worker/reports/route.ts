...
export async function GET(req:NextRequest){
 ...
 try{const {data,error}=await supabaseAdmin().from("grievances").select("*").neq("status","Completed").order("created_at",{ascending:true});if(error)throw error;return NextResponse.json({reports:data||[]})}catch(e){return NextResponse.json({error:e instanceof Error?e.message:"Database unavailable."},{status:500})}
}
